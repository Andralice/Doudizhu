...
function cd(c){
  ...
  var v=cv(c),s=cs(c);if(v===16)return{rank:'小',suit:'王',cls:'red'};if(v===17)return{rank:'大',suit:'王',cls:'red'};
...
// ---- State ----
var st={
  screen:'auth',tab:'login',loginAcc:'',loginName:'',loggingIn:false,autoLog:false,loginErr:'',
  acc:'',name:'',token:'',stats:null,joinId:'',homeErr:'',history:[],
  ...
  gPlayers:[null,null,null],toast:{show:false,msg:'',type:'info'}
};
var toastTmr=null;
...
var ws=null,rc=0;
function conn(){
  var p=location.protocol==='https:'?'wss:':'ws:';ws=new WebSocket(p+'//'+location.host);
...
function handle(m){
  ...
  switch(t){
    case'login_ok':st.token=p.token;st.acc=p.accountId;st.name=p.playerName;st.stats=p.stats||null;localStorage.setItem('dld_token',p.token);localStorage.setItem('dld_acc',p.accountId);localStorage.setItem('dld_name',p.playerName);st.loggingIn=false;st.autoLog=false;st.loginErr='';st.screen='home';toast('欢迎，'+p.playerName,'success');break;
    case'error':if(st.screen==='auth'){st.loggingIn=false;st.autoLog=false;st.loginErr=p.message}else toast(p.message,'error');break;
    case'room_joined':if(p.roomId)st.roomId=p.roomId;if(p.seat!==undefined)st.mySeat=p.seat;if(p.players){st.players=p.players;st.gPlayers=p.players}st.ready=false;st.roomErr='';st.screen='room';break;
    ...
    case'player_joined':case'player_left':case'player_ready':case'player_offline':case'player_reconnected':if(p.players){st.players=p.players;st.gPlayers=p.players}if(t==='player_joined')toast((p.isAI?'AI ':'')+p.playerName+' 加入',p.isAI?'info':'success');break;
    case'game_start':st.hand=sortH(p.hand||[]);st.phase='bidding';st.bottom=[];st.landlord=null;st.turn=null;st.lastPlay=null;st.myPlayed=[];st.leftPlayed=[];st.rightPlayed=[];st.sel=[];st.phaseText='发牌 · 叫地主';st.showBid=false;st.showDouble=false;st.showResult=false;st.screen='game';toast('游戏开始！','success');break;
    case'bid_turn':st.turn=p.seat;st.phase='bidding';if(p.seat===st.mySeat){st.showBid=true;st.bidMsg='请叫地主 (最高:'+(p.currentScore||0)+'分)';st.bidCur=p.currentScore||0}else{st.showBid=false;st.phaseText='叫地主 · 最高:'+(p.currentScore||0)+'分'}break;
    ...
    case'bid_result':st.showBid=false;st.landlord=p.landlordSeat;st.score=p.baseScore;st.bottom=p.bottomCards||[];st.phase='grabbing';st.phaseText=p.landlordName+' 暂为地主 · 抢地主阶段';break;
    case'grab_turn':st.turn=p.seat;st.phase='grabbing';if(p.seat===st.mySeat){st.showBid=true;st.bidMsg='是否抢地主？';st.bidCur=0}else{st.showBid=false;st.phaseText='抢地主中'}break;
    case'grab_update':if(p.passed)st.phaseText='座位'+(p.seat+1)+': 不抢';else{st.landlord=p.newLandlordSeat;st.score=p.baseScore;st.phaseText='座位'+(p.seat+1)+': 抢地主！'}break;
    case'grab_result':st.showBid=false;st.landlord=p.landlordSeat;st.bottom=p.bottomCards||[];st.score=p.baseScore;st.phase='doubling';st.phaseText=p.landlordName+' 是地主 · 加倍';if(p.landlordSeat===st.mySeat&&p.bottomCards)st.hand=sortH(st.hand.concat(p.bottomCards));break;
    case'double_turn':st.turn=p.seat;st.phase='doubling';if(p.seat===st.mySeat)st.showDouble=true;else st.phaseText='加倍...';break;
    ...
    case'play_turn':st.turn=p.seat;st.lastPlay=p.lastPlaySeat;st.phase='playing';if(p.isNewRound){st.myPlayed=[];st.leftPlayed=[];st.rightPlayed=[]}st.sel=[];break;
    case'play_result':if(p.seat===st.mySeat){var ps=new Set(p.cards);st.hand=st.hand.filter(function(c){return!ps.has(c)});st.myPlayed=p.cards}else if(p.seat===ls())st.leftPlayed=p.cards;else if(p.seat===rs())st.rightPlayed=p.cards;if(st.gPlayers[p.seat])st.gPlayers[p.seat].handCount=p.handCount;st.sel=[];break;
    case'pass_result':if(p.seat===st.mySeat)st.sel=[];break;
    case'hint_result':if(p.cards&&p.cards.length>0){st.sel=p.cards;toast('推荐出牌','info')}else toast('无牌可出','error');break;
    case'game_end':st.showResult=true;st.phase='finished';st.resTitle=p.isLandlordWin?'地主获胜！':'农民获胜！';st.resDetail='赢家:'+p.winnerName+' | '+p.baseScore+' × '+p.multiplier+' = '+p.finalScore+'分';var isL=st.landlord===st.mySeat,won=(p.isLandlordWin&&isL)||(!p.isLandlordWin&&!isL);st.history.unshift({date:new Date().toLocaleDateString(),won:won,detail:(won?'胜':'负')+' · '+(p.isLandlordWin?'地主赢':'农民赢')+' · '+p.finalScore+'分'});if(st.history.length>50)st.history.length=50;localStorage.setItem('dld_hist',JSON.stringify(st.history));if(p.playerStats&&p.playerStats[st.mySeat])st.stats=p.playerStats[st.mySeat];break;
    case'room_state':st.roomId=p.roomId;st.mySeat=p.seat;st.hand=sortH(p.hand||[]);st.turn=p.currentTurnSeat;st.landlord=p.landlordSeat;st.lastPlay=p.lastPlaySeat;st.bottom=p.bottomCards||[];st.score=p.baseScore;st.mult=p.multiplier;if(p.players){st.gPlayers=p.players;st.players=p.players}st.screen=p.phase==='waiting'?'room':'game';st.phase=p.phase==='playing'?'playing':p.phase==='bidding'?'bidding':p.phase==='doubling'?'doubling':'';if(st.screen==='game')st.phaseText='已重连';toast('已恢复','success');break;
  }
}
...
function doRegister(){var a=$('reg-acc'),n=$('reg-name');if(!a||!n)return;var acc=a.value.trim(),name=n.value.trim();if(!acc||!name){tx('reg-err','请填写账号和昵称');return}st.loggingIn=true;tx('reg-err','');st.acc=acc;st.name=name;send('register',{accountId:acc,nickname:name})}
function doLogin(){var a=$('login-acc');if(!a)return;var acc=a.value.trim();if(!acc){tx('login-err','请输入账号');return}st.loggingIn=true;tx('login-err','');send('login',{accountId:acc})}
...
function doPlay(){if(st.phase!=='playing'){toast('请先完成叫地主和加倍','error');return}if(st.sel.length===0){toast('请选择要出的牌','error');return}send('play',{cards:st.sel.slice()})}
...
function up(){
  ...
  if(s.screen==='auth'){
    dp('form-login',s.tab==='login'?'block':'none');dp('form-register',s.tab==='register'?'block':'none');
    var tl=$('tab-login'),tr=$('tab-register');if(tl)tl.className='auth-tab'+(s.tab==='login'?' active':'');if(tr)tr.className='auth-tab'+(s.tab==='register'?' active':'');
    tx('login-err',s.tab==='login'?s.loginErr:'');tx('reg-err',s.tab==='register'?s.loginErr:'');dp('auth-auto',s.autoLog?'block':'none');
  ...
  if(s.screen==='home'){
    ...
    var sp=$('stats-panel'),si=$('stats-info');if(sp&&si){if(s.stats){sp.style.display='block';si.innerHTML='<span>对局 '+s.stats.gamesPlayed+'</span><span>胜场 '+s.stats.gamesWon+'</span><span>胜率 '+s.stats.winRate+'</span>'}else sp.style.display='none'}
    var hp=$('hist-panel'),hl=$('hist-list');if(s.history.length>0){if(hp)hp.style.display='block';if(hl)hl.innerHTML=s.history.slice(0,8).map(function(h){return'<div class="item"><span class="dt">'+h.date+'</span><span class="rs '+(h.won?'win':'lose')+'">'+(h.won?'胜':'负')+'</span><span class="det">'+h.detail+'</span></div>'}).join('')}else{if(hp)hp.style.display='none'}
    tx('home-err',s.homeErr);
    fetch('/rooms').then(function(r){return r.json()}).then(function(rooms){var rl=$('rooms-list');if(!rl)return;var waiting=rooms.filter(function(r){return r.phase==='waiting'});if(waiting.length===0)rl.innerHTML='<p class="dim sm">暂无开放房间</p>';else rl.innerHTML=waiting.map(function(r){return'<div class="item"><span class="det">房间 '+r.roomId+'</span><span>'+r.playerCount+'/3人</span><button class="btn primary sm" onclick="st.joinId=\''+r.roomId+'\';doJoinRoom()">加入</button></div>'}).join('')}).catch(function(){});
  }
  dp('scr-room',s.screen==='room'?'flex':'none');
  if(s.screen==='room'){
    tx('room-id',s.roomId);for(var i=0;i<3;i++){renderSeat(i)}
    ...
    var ab=$('add-ai-btn');if(ab)ab.disabled=pc()>=3||!s.roomId;tx('room-err',s.roomErr);
  }
  dp('scr-game',s.screen==='game'?'block':'none');if(s.screen==='game')renderGame();
}
...
function renderGame(){
  try{
    ...
    // Turn hint
    var th=$('turn-hint');if(th){if(s.phase==='bidding')th.textContent=s.turn===s.mySeat?'轮到你叫地主':s.turn!==null?'等待叫地主...':'';else if(s.phase==='grabbing')th.textContent=s.turn===s.mySeat?'轮到你抢地主':s.turn!==null?'等待抢地主...':'';else if(s.phase==='doubling')th.textContent=s.turn===s.mySeat?'轮到你加倍':s.turn!==null?'等待加倍...':'';else if(s.phase==='playing')th.textContent=s.turn===s.mySeat?'轮到你出牌':s.turn!==null?'等待 '+gpn(s.turn)+' 出牌...':'';else th.textContent=''}

    // Buttons
    var playBtn=$('btn-play'),passBtn=$('btn-pass'),hintBtn=$('btn-hint');
    var myTurn=s.turn===s.mySeat&&s.phase==='playing';
    if(playBtn)playBtn.disabled=!(myTurn&&s.sel.length>0);if(passBtn)passBtn.disabled=!(myTurn&&s.lastPlay!==null&&s.lastPlay!==s.mySeat);if(hintBtn)hintBtn.disabled=!myTurn;

    // Overlays
    dp('ov-bid',s.showBid?'flex':'none');if(s.showBid){tx('bid-msg',s.bidMsg);var ovb=document.querySelector('#ov-bid .ov-btns');if(ovb){if(s.phase==='grabbing')ovb.innerHTML='<button class="btn secondary" onclick="doGrab(false)">不抢</button><button class="btn danger" onclick="doGrab(true)">抢地主</button>';else ovb.innerHTML='<button class="btn secondary" onclick="doBid(0)">不叫</button><button class="btn primary" onclick="doBid(1)">1分</button><button class="btn primary" onclick="doBid(2)">2分</button><button class="btn danger" onclick="doBid(3)">3分</button>';document.querySelectorAll('#ov-bid .btn').forEach(function(b){var sc=parseInt(b.textContent);if(sc>0)b.disabled=sc<=s.bidCur})}}
    dp('ov-double',s.showDouble?'flex':'none');dp('ov-result',s.showResult?'flex':'none');
...
window.doAddAI=doAddAI;window.doToggleReady=doToggleReady;
...
window.cardDown=cardDown;window.handMove=handMove;window.handUp=handUp;
